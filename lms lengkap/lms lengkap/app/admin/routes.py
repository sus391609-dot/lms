...
from app.models import (
    ...
    Berita, Kegiatan, KerjaSama, PendaftaranPMB,
)
...
# =====================================================================
# KONTEN PUBLIK: BERITA
# =====================================================================
def _parse_dt(value: str):
    """Parse 'YYYY-MM-DDTHH:MM' (datetime-local) ke datetime; None bila kosong."""
    if not value:
        return None
    try:
...
def berita_form(bid=None):
    ...
    if request.method == "POST":
        ...
        if not judul:
            ...
            return render_template("admin/berita_form.html", b=b)
...
def kegiatan_form(kid=None):
    ...
    if request.method == "POST":
        ...
        if not judul or not isi:
            ...
            return render_template("admin/kegiatan_form.html", k=k)
...
def kerjasama_form(kid=None):
    ...
    if request.method == "POST":
        ...
        if not judul or not mitra or not isi:
            ...
            return render_template("admin/kerjasama_form.html", ks=ks)
        ...
        return redirect(url_for("admin.kerjasama_list"))
    return render_template("admin/kerjasama_form.html", ks=ks)
...
def pmb_pendaftar():
    ...
    rows = query.order_by(PendaftaranPMB.created_at.desc()).limit(200).all()
    return render_template("admin/pmb_pendaftar.html", rows=rows, status=status)
...
def pmb_pendaftar_status(pid):
    ...
    return redirect(url_for("admin.pmb_pendaftar"))
